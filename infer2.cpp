...
// Generate samples
deque<deque<double>> Get_Sample(int sample_num, double S_u, double S_l, double I_u, double I_l, int N) {
    ...
    for (int i = 0; i < sample_num; i++) {//The first n-1 position in each sample is the susceptibility probability, the position N to 2*N is the infection probability, and the position 2*N to 3* n-1 is the recovery probability
        for (int j = 0; j < N; j++) {
...
deque<deque<int>>Get_repeat_time(deque<int>itime) {
    ...
    int l =int(itime.size()) - 1;
    non_repeat_day.push_back(itime[l]);
    repeat_position.push_back(l);
...
deque<deque<deque<double>>>Backward(int sample_num, deque<deque<int>> diagno, deque<deque<int>> Neighbor, deque<deque<double>>Sample_t, int infer_time, int N, double gama, deque<deque<double>>Sample_beta) {
    /////////initial
    deque<deque<deque<double>>>P_SIR;//100*diag_num*3
    for (int j = 0; j < sample_num; j++) {
        deque<deque<double>>iter2;
        for (int i = 0; i < diagno.size(); i++) {
            deque<double>iter1(3, 0);
            iter2.push_back(iter1);
        }
        P_SIR.push_back(iter2);
    }
    ...
    for (int i = 0; i < sample_num; i++) {
        deque<double>Sample_t_sam_S = Sample_t[i];
        deque<double>Sample_t_sam_I = Sample_t[i];
        deque<double>Sample_t_sam_R = Sample_t[i];
        deque<double>Sample_beta_sam= Sample_beta[i];
        for (int j = 0; j < diagno.size(); j++) {
            int nodenum = diagno[j][1];
        ...
        deque<double>lh_S(diagno.size(), 0), lh_I(diagno.size(), 0), lh_R(diagno.size(), 0);//Likelihood  vector.size=1*diag_num
        ...
        for (int k = 0; k < repeat_current[0].size() - 1; k++) {
            for (int t = repeat_current[0][k]; t < repeat_current[0][k + 1]; t++) {
                Sample_t_sam_S = Model_Integ(Sample_t_sam_S, Neighbor, N, gama, Sample_beta_sam);
                Sample_t_sam_I = Model_Integ(Sample_t_sam_I, Neighbor, N, gama, Sample_beta_sam);
                Sample_t_sam_R = Model_Integ(Sample_t_sam_R, Neighbor, N, gama, Sample_beta_sam);
            }
            for (int ii = repeat_current[1][k]; ii < repeat_current[1][k + 1]; ii++) {
                lh_S[ii] = Sample_t_sam_S[diagno[ii][1] + diagno[ii][2] * N];
                lh_I[ii] = Sample_t_sam_I[diagno[ii][1] + diagno[ii][2] * N];
                lh_R[ii] = Sample_t_sam_R[diagno[ii][1] + diagno[ii][2] * N];
                if (diagno[ii][2] == 0) {
                    P_SIR[i][ii][0] = 1;
                    P_SIR[i][ii][1] = 0;
                    P_SIR[i][ii][2] = 0;
                ...
                if (diagno[ii][2] == 1) {
                    double sum = Sample_t[i][diagno[ii][1]] * lh_S[ii] + Sample_t[i][diagno[ii][1] + N] * lh_I[ii];
                    P_SIR[i][ii][0] = Sample_t[i][diagno[ii][1]] * lh_S[ii] / sum;
                    P_SIR[i][ii][1] = Sample_t[i][diagno[ii][1] + N] * lh_I[ii] / sum;
                    P_SIR[i][ii][2] = 0;
                ...
                if (diagno[ii][2] == 2) {
                    double sum = Sample_t[i][diagno[ii][1]] * lh_S[ii] + Sample_t[i][diagno[ii][1] + N] * lh_I[ii] + Sample_t[i][diagno[ii][1] + 2 * N] * lh_R[ii];
                    P_SIR[i][ii][0] = Sample_t[i][diagno[ii][1]] * lh_S[ii] / sum;
                    P_SIR[i][ii][1] = Sample_t[i][diagno[ii][1] + N] * lh_I[ii] / sum;
                    P_SIR[i][ii][2] = Sample_t[i][diagno[ii][1] + 2 * N] * lh_R[ii] / sum;
                    Sample_t_sam_S[diagno[ii][1]] = 0;
                    Sample_t_sam_S[diagno[ii][1] + N] = 0;
        ...
        Sample_t_sam_S.clear();
        Sample_t_sam_I.clear();
        Sample_t_sam_R.clear();
        Sample_beta_sam.clear();
        lh_S.clear();
        lh_I.clear();
        lh_R.clear();
    }
    diagno_new_time.clear();
    repeat_current.clear();
    return P_SIR;
}
...
int main() {
    ...
    double I_u = 1 - S_l;//up bound of infected
    double I_l = 1 - S_u;//lower bound of infected
    ...
    deque<deque<double>>Sample_0 = Get_Sample(sample_num, S_u, S_l, I_u, I_l, N);//100*(3N)
    ...
    for (int t = T_start; t < T_end; t++) {
        ...
        deque<deque<deque<double>>>post_Inference = Backward(sample_num, diagno_current, Neighbor, Sample[t], t, N, gama, Sample_beta);//Inference
